...
import type { AgendaExamen, AgendaNovedad, TipoPublicacion } from '../../types';
...
function diasRestantes(iso: string): string {
  ...
  const dd = new Date(d);
  dd.setHours(0, 0, 0, 0);
  const dias = Math.round((dd.getTime() - hoy.getTime()) / 86400000);
  if (dias <= 0) return 'Hoy';
  if (dias === 1) return 'Mañana';
  return `En ${dias} días`;
}

// ── Tarjeta: próximos exámenes ──────────────────────────────────────────────

export function ProximosExamenes({ examenes }: { examenes: AgendaExamen[] }) {
  return (
    <Card>
...
// ── Tarjeta: novedades del foro ─────────────────────────────────────────────

export function NovedadesForo({ novedades }: { novedades: AgendaNovedad[] }) {
  return (
    <Card>
      <div className="mb-3 flex items-center gap-2">
      ...
      {novedades.length === 0 ? (
        ...
        <ul className="divide-y divide-slate-100">
          {novedades.map((n) => (
            <li key={n.id}>
              <Link to={`/materias/${n.materia.id}`} className="group flex items-start gap-3 py-2.5">
...
// ── Combinado (autónomo) — usado en el dashboard del docente ────────────────
